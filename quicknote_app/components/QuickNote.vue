...
function getContentSnippet(content: string, maxLength: number = 100): string {
...
<template>
  <div class="quicknote-app" style="background-color: #ffffff; min-height: 100vh; padding: 2rem;">
    <!-- Header -->
    <div class="header" style="margin-bottom: 2rem;">
      <h1 style="color: #333; font-size: 2.5rem; font-weight: bold; margin-bottom: 1rem; text-align: center;">
        📝 QuickNote
      </h1>
    ...
    <!-- Notes Grid -->
    <div v-if="filteredNotes.length > 0" class="notes-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1.5rem; max-width: 1200px; margin: 0 auto;">
      <div
    ...
    <!-- Empty State -->
    <div v-else-if="!searchQuery" style="text-align: center; margin-top: 4rem; color: #666;">
      <div style="font-size: 4rem; margin-bottom: 1rem;">📝</div>
      <h3 style="font-size: 1.5rem; margin-bottom: 1rem;">No notes yet</h3>
      <p style="font-size: 1rem; margin-bottom: 2rem;">Create your first note to get started!</p>
      <button
        @click="showAddNoteForm"
        style="background-color: #4a90e2; color: white; border: none; border-radius: 8px; padding: 12px 24px; font-size: 16px; cursor: pointer;"
      >
        Create Note
      </button>
    </div>
    ...
    <div
      v-if="showAddForm"
      style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000; padding: 2rem;"
      @click.self="hideForm"
    >
      <div style="background-color: white; border-radius: 12px; padding: 2rem; width: 100%; max-width: 600px; max-height: 80vh; overflow-y: auto;">
